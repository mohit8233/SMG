...
export default function AdminStudents() {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      // 1. User create karo
      const userRes = await authAPI.register({ name: form.name, email: form.email, password: form.password, role: "student", phone: form.phone });
      // 2. User no _id lavva login karo (ya getAll users)
      // Simplified: direct student create
      const loginRes = await authAPI.login({ email: form.email, password: form.password });
      const userId = loginRes.data.user._id;
      await studentAPI.create({ userId, rollNumber: form.rollNumber, course: form.course, semester: Number(form.semester) });
      setShowModal(false); setForm(initForm); fetchStudents();
    } catch (err) {
